...
class TestCheckMandatoryParam(unittest.TestCase):
    ...
    def test_missing_common_param(self):
        ...
        for p in ['subject', 'sender', 'sendername', 'message']:
            ...
            self.assertFalse(sendMail.check_mandatory_param(param_obj), f"Should fail when {p} is missing")
    ...
    def test_smtp_mode_missing_param(self):
        ...
        smtp_params = ['smtp_port', 'imap_host', 'imap_port', 'username', 'password', 'sent_folder']
        for p in smtp_params:
            ...
            del params[p]
            param_obj = sendMail.Dict2Class(params)
            self.assertFalse(sendMail.check_mandatory_param(param_obj), f"Should fail when {p} is missing in SMTP mode")
    ...
    def test_gmail_mode_missing_param(self):
        ...
        for p in gmail_params:
            ...
            del params[p]
            param_obj = sendMail.Dict2Class(params)
            self.assertFalse(sendMail.check_mandatory_param(param_obj), f"Should fail when {p} is missing in Gmail mode")
